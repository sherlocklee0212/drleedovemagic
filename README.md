...
它不會出現在文章頁面上，只做兩件幕後的事：

1. 首頁、分類頁、全部文章的卡片摘要
2. 把連結貼到 LINE、Facebook 時顯示的那段預覽文字

所以**不用怕跟第一段重複**，讀者不會同時看到兩次。

如果某篇文章比較長，想在標題下面放一句引言，再多加一行 `lede:`：

```markdown
---
title: 第一次放飛
lede: 準備了三個月，結果牠在門口站了十分鐘不肯動。
categories: [training]
description: 一句話說明這篇在寫什麼。用在首頁卡片與分享預覽，不會印在文章裡。
---
```

沒寫 `lede:` 就不會有引言，短文章通常不需要。
